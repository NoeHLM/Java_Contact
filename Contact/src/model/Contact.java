...
public class Contact {
    ...
    public static void ecrire(ArrayList<Contact> liste) throws IOException {
       ...
       SimpleDateFormat dtf = new SimpleDateFormat("dd/MM/yyyy");
       try {
           writer = new BufferedWriter(new FileWriter("contacts.csv"));
           // Écrivez chaque contact de la liste dans le fichier contacts.csv
           for (Contact contact : liste) {
               try {
                   writer.write(contact.getNom() + ";" + contact.getPrenom() + ";" + contact.getMail() + ";"
                           + contact.getNumero() + ";" + dtf.format(contact.getDateNaissance()) + "\n");
               } catch (Exception e) {
